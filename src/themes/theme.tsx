...
export const ThemeProvider: React.FC<{children: ReactNode}> = ({children}) => {
    // Init themes
    const [cachedThemes, setCachedThemes] = 
    useState<Record<string, WallpaperTheme>>(() => {
        try {
    ...
    const [currentThemeId, setCurrentThemeId] = useState<string>(() => {
        return localStorage.getItem('portfolio_active_theme_id') || DEFAULT_THEME.id;
    });
    ...
    useEffect(() => {
        try {
            localStorage.setItem("portfolio_theme_cache", JSON.stringify(cachedThemes));
        } catch (error) {
    ...
    const generateAndCacheTheme = async (id: string, name: string, imageUrl: string) => {
        if (themeSet.has(id)) {
            setTheme(id);
            return;
        }
